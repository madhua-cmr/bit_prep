...
export default async function getInterviewDetailsById(id:string):Promise<Interview|null>{
...
export async function generateFeedback({userId,interviewId,transcript}:CreateFeedbackParams){
  try {
  const formattedTranscript=transcript.map((transcript:{role:string,content:string})=>(
  ...
  const {object:{totalScore,categoryScores,strengths,areasForImprovement,finalAssessment}}=await generateObject({
    ...
    //return the refernce to the document
